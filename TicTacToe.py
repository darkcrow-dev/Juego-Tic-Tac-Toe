...
class TicTacToe:
    ...
    def jugar(self):
        ...
        if(self.opciones[self.modalidad - 1] == "1 JUGADOR"):
            ...
            self.tablero(self.matrizTablero)
            self.turnos()
    ...
    def ganador(self, fichaTurno, numeroMovimiento, matrizTablero):
    ...
    def movimientoJugador(self):
        ...
        if( (casillas > 0) and (casillas <10) ):
            if( (self.matrizTablero[filas][columnas]) == " "):
                ...
                self.tablero(self.matrizTablero)
                self.palabras = self.ganador(self.fichas[self.turno], self.contadorMovimientos, self.matrizTablero)
                self.turno = int(not self.turno)
